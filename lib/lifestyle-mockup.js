...
function buildPrompt({ productDesc, scene, angle, mode, idx, total }) {
  if (mode === 'bg-replace') {
    return `Edit this product photo: REPLACE ONLY THE BACKGROUND and CHANGE THE CAMERA ANGLE. The product in the image is a ${productDesc}.

NEW BACKGROUND: ${scene}
CAMERA ANGLE for this shot: ${angle}

CRITICAL RULES:
1. Keep the product's identity, colors, materials, design, and proportions PIXEL-FAITHFUL to the input. Same product, same finish.
2. Recompose the scene from the new camera angle described above. Re-render lighting, shadows, perspective, and surroundings naturally for that angle.
3. Background and props must match the SCENE description. Add small contextual props around (not on top of) the product for a lifestyle Etsy listing look.
4. Lighting on the product must match the new background and angle naturally.
5. Square 1:1 output.
6. DO NOT add any text, watermarks, labels, or logos.

Mockup ${idx} of ${total} - this view must be visually distinct from every other mockup in the set.`;
  }
  return `These images show a product (${productDesc}). Create a professional Etsy listing mockup photo. This is mockup ${idx} of ${total} -- each one must have a unique camera angle AND unique styled background, never repeat.

SCENE: ${scene}
CAMERA ANGLE: ${angle}

The mockup must look like a top-selling Etsy shop's listing photo -- warm natural lighting, lifestyle-styled with complementary props and textures, cozy and inviting atmosphere. The product is the hero of the image, captured from the specified camera angle above.

RULES:
1. Square 1:1 output.
2. The product MUST be placed compositionally as the focal point (dead center if angle allows; otherwise hero placement).
