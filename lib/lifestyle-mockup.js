...
5. Square 1:1 output. No text, watermarks, labels, or logos.`;
  ...
  return `Reference photos show a product: ${productDesc}.

YOUR TASK: Re-photograph this same product from a new camera angle, in a new lifestyle scene. This is mockup ${idx} of ${total} -- it MUST be visually distinct from every other mockup in the set (different angle, different scene, different framing).

SCENE: ${scene}
CAMERA ANGLE: ${angle}

OUTPUT REQUIREMENTS:
1. Square 1:1 image, photo-realistic, professional Etsy listing quality.
2. Re-render the product from the specified CAMERA ANGLE. The angle MUST visibly change between mockups -- a flat lay must look like a flat lay, a low angle must look like a low angle, etc. Do NOT just shift the background while keeping the same view.
3. Preserve the product's identity: same item, same colors, same materials, same design, same proportions, same finish. The product must remain recognizable as the input product, but seen from the new angle.
4. SCENE composition: warm natural lighting, lifestyle-styled props that fit the scene description above. The product is the hero of the image.
5. NO text, watermarks, labels, logos, signatures, or any writing.
6. The product is the focal point but it does NOT have to be dead-center — frame it naturally for the chosen camera angle (e.g., off-center for 3/4 angle, edge-of-frame for in-use scenes).
...
async function generateLifestyleMockups({ productImagePath, productImagePaths, productDescription, sku, count = 10, mode = 'rotate-angles', onProgress }) {
  ...
  // Niche overrides scene pool if defined (niches/<slug>.json -> scenePool: [...])
  ...
  for (let i = 0; i < count; i++) {
    ...
    concepts.push({ angle: angleLabel, scene });
    onProgress?.({ type: 'mockup-start', idx: i + 1, total: count, angle: angleLabel, scene });
    const primary = imgParts[i % imgParts.length];
    const refs = imgParts.length > 1
      ? [primary, ...imgParts.filter(p => p !== primary).slice(0, 2)]
      : [primary];
    const prompt = buildPrompt({ productDesc: productDescription, scene, angle: angleDesc, mode, idx: i + 1, total: count });
    try {
      ...
      onProgress?.({ type: 'mockup-done', idx: i + 1, total: count, path: relPath, angle: angleLabel, scene });
    } catch (err) {
      console.warn(`[lifestyle-mockup] idx=${i + 1} failed: ${err.message}`);
      onProgress?.({ type: 'mockup-error', idx: i + 1, total: count, error: err.message, angle: angleLabel });
    }
  }
